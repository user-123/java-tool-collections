...
public class ImageUtil {
	...
	public enum COMPARISON_OPTION {
		...
		private int option;
		protected int compare() {return this.option;}
	}
	...
	public static boolean checkImageSize(BufferedImage image, int targetWidth, int targetHeight, COMPARISON_OPTION comparisonOption) {
		return !isNullOrEmpty(image) && !isIllegalResolution(targetWidth, targetHeight) && comparisonOption!=null && switch (comparisonOption.compare()) {
			case 0 -> isSameSize(image, targetWidth, targetHeight);
			case 1 -> image.getWidth()>targetWidth && image.getHeight()>targetHeight;
			case -1 -> image.getWidth()<targetWidth && image.getHeight()<targetHeight;
			default -> throw new IllegalStateException("Unexpected value: " + comparisonOption.compare());
		};
	}
}
